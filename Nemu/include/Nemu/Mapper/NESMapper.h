...
namespace nemu
{
template <class Iterator> class NESMapper {
	...
	static MaskIterator<Iterator> APUIORegisters(const Iterator &it,
						     std::size_t offset)
	{
	...
	NESMapper(const Iterator &it, difference_type offset)
		: it(it), offset(offset)
	{
	}
	...
	NESMapper<Iterator> &operator++()
	{
		++offset;
		return *this;
	}

	NESMapper<Iterator> &operator+=(difference_type n)
	{
		offset += n;
		return *this;
	}
	...
	bool operator!=(const NESMapper<Iterator> &other)
	{

...
public:
	...
	std::uint8_t ToBitMask(std::uint8_t* array)
	{
		std::uint8_t mask = 0;
	...
	std::vector<std::uint8_t> FromBitMask(std::uint8_t mask)
	{
		std::vector<std::uint8_t> array(8);
		for (unsigned int i = 0; i < 8; i++)
			array[i] = (1 << i) & mask == (1 << i);
		return array;
	}
	...
	bool Decode()
	{
		...
		case 0x0: { // BRK
			...
			stack.Push(instruction_memory[reg_pc + 2]);
		...
		case 0xF0: { // BEQ
			std::uint8_t oper = instruction_memory[reg_pc + 1];
			if (reg_status[S_ZERO])
				reg_pc = *(std::uint16_t*)(instruction_memory.data() + reg_pc + oper);
			else
				reg_pc += 2;
			return true;
		}
